...
export const pets_feature = register("command", username => {
    ...
    const pet_thing = new Map()//tyvm to eatplastic for the sorting pets by rarity code
    pet_thing.set("COMMON", "")
    pet_thing.set("UNCOMMON", "")
    pet_thing.set("RARE", "")
    pet_thing.set("EPIC", "")
    pet_thing.set("LEGENDARY", "")
    pet_thing.set("MYTHIC", "")
    ...
    g_rank(username,apikey).then(rank_data => {
      ...
      get_profile_id(user_uuid, null, apikey).then(res => {
        ...
        get_pets(username,cute_name).then(pets_data => { 
          ...
          if(hname.includes("Vampire Fang") && ptier.includes("LEGENDARY") && pname.includes("Bat")) ptier = "MYTHIC", p_lvl_color = colors[11];
          if(hname.includes("Jerry 3D Glasses") && ptier.includes("LEGENDARY") && pname.includes("Jerry")) ptier = "MYTHIC", p_lvl_color = colors[11];
          ...
          pet_thing.set(ptier, pet_thing.get(ptier) + "*" + (`${tcolor}[Lvl ${plvl}] ${clean_pname} ;${hmsg}`))
          i++;
        }
        pet_thing.forEach(key => {
          if(key.length>0) {
            key.split("*").forEach(text => {
              if(text!="") {
                hover_msg(text.split(";")[0],text.split(";")[1])
              }
            })
          }
        })
